package me.iantapply.dungeons.developerkit.utils.nbt;
...
public enum MinecraftVersion {
	 ...
	 */
	private static Logger logger = Logger.getLogger("Nostos");

	// NBT-API Version
	protected static final String VERSION = "2.10.0";
	...
	public static MinecraftVersion getVersion() {
		...
		if (version != UNKNOWN) {
			...
			//logger.warning("This Server-Version(" + ver + ") is not supported by this NBT-API Version(" + VERSION + ") located at " + MinecraftVersion.class.getName() + ". The NBT-API will try to work as good as it can! Some functions may not work!");
		}
		init();
		return version;
	}
		...
		if (hasGsonSupport() && !updateCheckDisabled)
			new Thread(() -> {
				try {
					...
					logger.log(Level.WARNING, "Error while checking for updates! Error: " + ex.getMessage());
				}
			}).start();
